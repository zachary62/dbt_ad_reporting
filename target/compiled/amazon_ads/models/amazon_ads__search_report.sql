...
with report as (
    select *
    from "postgres"."public_amazon_ads_source"."stg_amazon_ads__search_term_ad_keyword_report"
), 

account_info as (
    select *
    from "postgres"."public_amazon_ads_source"."stg_amazon_ads__profile"
    where _fivetran_deleted = False
),

portfolios as (
    select *
    from "postgres"."public_amazon_ads"."int_amazon_ads__portfolio_history"
), 

campaigns as (
    select *
    from "postgres"."public_amazon_ads_source"."stg_amazon_ads__campaign_history"
    where is_most_recent_record = True
),

ad_groups as (
    select *
    from "postgres"."public_amazon_ads_source"."stg_amazon_ads__ad_group_history"
    where is_most_recent_record = True
), 

keywords as (
    select *
    from "postgres"."public_amazon_ads_source"."stg_amazon_ads__keyword_history"
    where is_most_recent_record = True
), 
